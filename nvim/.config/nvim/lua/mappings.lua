...
if not neovim.is_vscode() then
  ...
  local function indexOrNextProject(can_go_next)
    ...
    if (index < 0) then
      if can_go_next then require("monorepo").next_project() end
      return
    end
  ...
  maps.n["<leader>nt"] = {
    function() require("monorepo").toggle_project() end,
    desc = "Monorepo toggle project"
  }
  maps.n["<leader>nn"] = {
    function() indexOrNextProject() end,
    desc = "Monorepo next project or prefixed index"
  }
  maps.n["<C-m>"] = {
    function() indexOrNextProject(false) end,
    desc = "Monorepo next project or prefixed index"
  }
  ...
  local function runToggleTerm(args)
    ...
    if not args then args = "direction=float" end

    vim.cmd(":ToggleTerm " .. count .. args)
  end

  maps.n["<leader>tf"] = {
    function() runToggleTerm() end,
  ...
  maps.n["<leader>tv"] = {
    function() runToggleTerm("size=80 direction=vertical") end,
  ...
  maps.n["<leader>gj"] = {
    function()
      ...
      vim.cmd(".!quicktype -l " .. language ..
        " --no-enums --just-types --top-level " .. top_level)
    end,

...
local run_cmd = function(task, params, terminal_id, init_idf)
	...
	local cmd = string.format("UUID=%s %s %s'", workspace_uuid, task, params)
...
local show_info = function()
	local info = "PIO workspace info: \nSelected: device: "
		...
		.. "\nSelected env: "
		.. defaultNoneString(selected_env)
...
local get_serial_devices = function()
	...
	for _, device in ipairs(devices) do
		if
			string.find(device, "ttyUSB")
			or string.find(device, "ttyS")
			or string.find(device, "ttyACM")
			or string.find(device, "cu.usbserial")
			or string.find(device, "cu.PL2303")
			or string.find(device, "cu.usbmodem")
			or string.find(device, "cu.wch")
		then
...
local upload_fs = function()
	...
	vim.cmd(
		"6TermExec direction=float cmd='\rclear\rUUID=" .. workspace_uuid .. " pio run -t uploadfs " .. params .. "'"
	)
end
...
local run_simulator = function()
	...
	local cmd = string.format("UUID=%s cd %s/build/ ; make -j ; cd %s ; ./bin/main'", workspace_uuid, cwd, cwd)
...
local add_library = function()
	...
	if library == nil or library == "" then
		return
	end

...
local get_arduino_devices = function()
	...
	for _, board in pairs(boards_data) do
		if
			string.find(board.port, "usbserial")
			or string.find(board.port, "PL2303")
			or string.find(board.port, "usbmodem")
		then
...
local open_monitor = function()
	...
	vim.cmd(
		"5TermExec direction=float cmd='UUID="
			.. workspace_uuid
			.. " pio device monitor "
			.. params
			.. " --filter colorize'"
...
local add_library = function()
	...
	vim.cmd(
		"8TermExec direction=float cmd='\rclear\rUUID=" .. workspace_uuid .. " pio pkg install -l  " .. library .. "'"
	)
end
